...
parser.add_argument("--snr_threshold", "-t", default=10, help="SNR threshold for seticore", type=float)
parser.add_argument("--n_files", "-f", default=0, help="Number of files to analyze", type=int)
...
DR = args.dr ; N_DR = 200
if len(DR) == 1: DR = MIN_DR = MAX_DR = DR[0]
if len(DR) >= 2:
    MIN_DR = DR[0]
    MAX_DR = DR[1]
    if len(DR) == 3:
        N_DR = DR[2]
    if len(DR) >= 4: parser.error("You must provide 1 to 3 dr floats, not more")
    DR = None

SNR = args.dr ; N_SNR = 200
if len(SNR) == 1: SNR = MIN_SNR = MAX_SNR = SNR[0]
if len(SNR) >= 2:
    MIN_SNR = SNR[0]
    MAX_SNR = SNR[1]
    if len(SNR) == 3:
        N_SNR = SNR[2]
    if len(SNR) >= 4: parser.error("You must provide 1 to 3 dr floats, not more")
    SNR = None
...
def process_file(filepath):
    ...
    if already_processed(filepath): return "Already processed"
    try:
        print("  injecting signal") # injected describes the injected signal
        clean_path, injected_path, injected = inject_signal(filepath, OUTPUT_FOLDER, min_dr=MIN_DR, max_dr=MAX_DR, n_dr=N_DR, dr=DR, snr=SNR, min_snr=MIN_SNR, max_snr=MAX_SNR, n_snr=N_SNR)
        print("  running seticore")
        print(str(SNR_THRESH))
        subprocess.run(["seticore", clean_path, "-m", str(MIN_DR - .2), "-M", str(MAX_DR + .2),
                        "-s", str(SNR_THRESH), "--output", out_clean := (clean_path[:-3] + ".dat")],
                       check=True, env=seticore_env)
        subprocess.run(["seticore", injected_path, "-m", str(MIN_DR - .2), "-M", str(MAX_DR + .2),
                        "-s", str(SNR_THRESH), "--output", out_mod := (injected_path[:-3] + ".dat")],
                       check=True, env=seticore_env)
...
if __name__ == "__main__":
    batch_size = N_PROC
    if N_FILES > 0: all_files = all_files[:N_FILES]
